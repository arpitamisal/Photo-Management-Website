   ...
   sharp(uploadedFile)
        ...
        .then(function(){
            ...
            INSERT INTO posts (title, description, image, 
                thumbnail, fk_authorId) VALUE (?,?,?,?,?)
            `
            return db.query(baseSQL,[title, description, uploadedFile, 
            destinationOfThumbnail, userId])
        })
        .then(function([results,fields]){
            if(results && results.affectedRows){
                req.flash("success", "Your post has been creaated!");
...
router.get("/search", function(req, res, next){
    ...
    .then(function([results, fields]){
        ...
        req.session.save(function(saveErr){
            res.render('index', { css:["stylesheet.css"],
            js:["indexscript.js"]});
        })
    })
    });
